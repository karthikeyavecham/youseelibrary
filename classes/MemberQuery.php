<?php
/*
 * This file is part of a copyrighted work; it is distributed with NO WARRANTY. See the file COPYRIGHT.html for more details.
 */
...
class MemberQuery extends Query {
	 ...
	 * 
	 * @param string $type
	 *        	one of the global constants
	 *        	OBIB_SEARCH_BARCODE or OBIB_SEARCH_NAME
	 * @param string $word
	 *        	String to search for
	 * @param integer $page
	 *        	What page should be returned if results are more than one page
	 * @access public
	 *         ***************************************************************************
	 */
	 ...
	 * 
	 * @param string $mbrid
	 *        	Member id of library member to select
	 * @return boolean returns false, if error occurs
	 * @access public
	 *         ***************************************************************************
	 */
	function get($mbrid) {
		$sql = $this->mkSQL ( "select member.*, staff.username from member " . "left join staff on member.last_change_userid = staff.userid " . "where mbrid=%N ", $mbrid );
		$rows = $this->exec ( $sql );
		if (count ( $rows ) != 1) {
	...
	function maybeGetByBarcode($bcode) {
		$sql = $this->mkSQL ( "select member.*, staff.username from member " . "left join staff on member.last_change_userid = staff.userid " . "where barcode_nmbr=%Q ", $bcode );
	...
	function setCustomFields($mbrid, $fields) {
		...
		foreach ( $fields as $code => $data ) {
			$sql = $this->mkSQL ( 'insert into member_fields (mbrid, code, data) ' . 'values (%N, %Q, %Q)', $mbrid, $code, $data );
			$this->exec ( $sql );
		}
	}
	...
	function DupBarcode($barcode, $mbrid = 0) {
		$sql = $this->mkSQL ( "select count(*) as num from member " . "where barcode_nmbr = %Q and mbrid <> %N", $barcode, $mbrid );
		$rows = $this->exec ( $sql );
		if (count ( $rows ) != 1) {
	 ...
	 * 
	 * @param string $email
	 *        	Library member email address
	 * @param string $mbrid
	 *        	Library member id
	 * @return boolean returns true if email already exists
	 * @access private
	 *         ***************************************************************************
	 */
	function DupEmail($email, $mbrid = 0) {
		$sql = $this->mkSQL ( "select count(*) as num from member " . "where email = %Q and mbrid <> %N", $email, $mbrid );
		$rows = $this->exec ( $sql );
		if (count ( $rows ) != 1) {
			Fatal::internalError ( 'Bad number of rows' );
		}
	...
	/**
	 * **************************************************************************
	 * Returns true if work phone address already exists
	 * 
	 * @param string $phone
	 *        	Library member work phone
	 * @param string $mbrid
	 *        	Library member id
	 * @return boolean returns true if work phone already exists
	 * @access private
	 *         ***************************************************************************
	 */
	function DupPhone($phone, $mbrid = 0) {
		$sql = $this->mkSQL ( "select count(*) as num from member " . "where work_phone = %Q and mbrid <> %N", $phone, $mbrid );
		$rows = $this->exec ( $sql );
		if (count ( $rows ) != 1) {
			Fatal::internalError ( 'Bad number of rows' );
		}
	 ...
	 * 
	 * @param Member $mbr
	 *        	library member to insert
	 * @return integer the id number of the newly inserted member
	 * @access public
	 *         ***************************************************************************
	 */
	function insert($mbr) {
		$sql = $this->mkSQL ( "insert into member " . "(mbrid, barcode_nmbr, create_dt, last_change_dt, " . " last_change_userid, last_name, first_name, address, " . " home_phone, work_phone, email, classification) " . "values (null, %Q, sysdate(), sysdate(), %N, " . " %Q, %Q, %Q, %Q, %Q, %Q, %Q) ", $mbr->getBarcodeNmbr (), $mbr->getLastChangeUserid (), $mbr->getLastName (), $mbr->getFirstName (), $mbr->getAddress (), $mbr->getHomePhone (), $mbr->getWorkPhone (), $mbr->getEmail (), $mbr->getClassification () );
	 ...
	 * 
	 * @param Member $mbr
	 *        	library member to update
	 * @access public
	 *         ***************************************************************************
	 */
	function update($mbr) {
		$sql = $this->mkSQL ( "update member set " . " last_change_dt = sysdate(), last_change_userid=%N, " . " barcode_nmbr=%Q,  last_name=%Q,  first_name=%Q, " . " address=%Q, home_phone=%Q, work_phone=%Q, " . " email=%Q, classification=%Q " . "where mbrid=%N", $mbr->getLastChangeUserid (), $mbr->getBarcodeNmbr (), $mbr->getLastName (), $mbr->getFirstName (), $mbr->getAddress (), $mbr->getHomePhone (), $mbr->getWorkPhone (), $mbr->getEmail (), $mbr->getClassification (), $mbr->getMbrid () );
	 ...
	 * 
	 * @param string $mbrid
	 *        	Member id of library member to delete
	 * @access public
	 *         ***************************************************************************
	 */
